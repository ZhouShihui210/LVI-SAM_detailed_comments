...
/**
 * @brief   处理图像特征数据
 * @Description addFeatureCheckParallax()添加特征点到feature中，计算特征点跟踪到的次数和视差，判断是否是关键帧
 *              判断并进行外参标定
...
// 添加之前检测到的特征点到feature容器中，根据跟踪到的次数判断是否为关键帧
// td为imu和cam的时间差
// 最新的帧cnt为1，不能进行三角化
if (f_manager.addFeatureCheckParallax(frame_count, image, td))
marginalization_flag = MARGIN_OLD; //次新帧是关键帧
else
marginalization_flag = MARGIN_SECOND_NEW; //次新帧是关键帧

// Marginalize old imgs if lidar odometry available for initialization
// lidar里程计可用的话则把次新帧设为关键帧
if (solver_flag == INITIAL && lidar_initialization_info[0] >= 0)
marginalization_flag = MARGIN_OLD;

Headers[frame_count] = header;
//将图像数据 lio提供的初始位姿 时间戳以及预积分的值存储到图像帧类中
